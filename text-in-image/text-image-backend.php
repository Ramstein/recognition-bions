...
if (isset($_FILES['file'])) { # File Specifications (Name)
	...
	if ($s3_status === 200) { # Call textinImage Method for the Uploaded Image
		$result = $rekognition->textInImage($file_name); # Check Status of textInImage() return value
...
if (isset($_POST['demo-image'])) { # Read demo image
	...
	fclose($fp_image); # Call textInImageLocalFileSystem() and pass bytes of the image
	$result = $rekognition->textInImageLocalFileSystem($image); # Check Status of facialAnalysis
	$result_status = $result['@metadata']['statusCode']; # If successful return JSON of Output
	if ($result_status === 200) {
		json_output($result['TextDetections']);
	} else {
