...
if (isset($_FILES['file'])) { # File Specifications (Name)
	...
	if ($s3_status === 200) { # Call facialAnalysis Method for the Upload Image
		$result = $rekognition->imageModeration($file_name); # Check Status of facialAnalysis
...
if (isset($_POST['demo-image'])) { # Read demo image
	...
	fclose($fp_image); # Call imageModerationLocalFileSystem() and pass bytes of the image
	$result = $rekognition->imageModerationLocalFileSystem($image); # Check Status of facialAnalysis
	$result_status = $result['@metadata']['statusCode']; # If successful return JSON of Output
	if ($result_status === 200) {
		json_output($result['ModerationLabels']);
	} else {
