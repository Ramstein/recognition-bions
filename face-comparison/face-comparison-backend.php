<?php
/* *=========================================================================* Original Filename: face-comparison-backend.php * Last Modified Date: 15 March 2020 * */ # Include main Rekognition Class
...
if (isset($_FILES['file']) && isset($_FILES['file-comparison'])) { # File Specifications (Name)
	$file_name = $_FILES['file']['name'];
	$file_name_comparison = $_FILES['file-comparison']['name']; # Call s3Upload Method to Upload Image
	$s3_status = $rekognition->s3Upload($_FILES['file']);
	$s3_status_comparison = $rekognition->s3Upload($_FILES['file-comparison']); # If successfully uploaded to S3 Bucket
	if (($s3_status === 200) && ($s3_status_comparison === 200)) { # )Call facialAnalysis Method for the Upload Image
		$result = $rekognition->faceComparison($file_name, $file_name_comparison); # Check Status of facialAnalysis
...
if (isset($_POST['demo-image']) && isset($_POST['demo-image-comparison'])) { # Read demo images
	...
	fclose($fp_image_comparison); # Call faceComparisonLocalFileSystem() and pass bytes of the images
	$result = $rekognition->faceComparisonLocalFileSystem($image, $image_comparison); # Check Status of facialAnalysis
	$result_status = $result['@metadata']['statusCode']; # If successful return JSON of Output
	if ($result_status === 200) {
		json_output($result['FaceMatches']);
	} else {
